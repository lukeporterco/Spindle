...
public final class MinecraftLaunchPlanBuilder {
  public MinecraftLaunchPlan build(
    ...
    Path clientJar =
        minecraftDirectory == null
            ? null
            : installLocator.clientJarPath(minecraftDirectory, metadata.id());

    List<MinecraftLaunchPlan.Library> libraries =
        toPlanLibraries(workingDirectory, librarySelection.libraries());
    List<MinecraftLaunchPlan.Library> nativeLibraries =
        toPlanLibraries(workingDirectory, librarySelection.nativeLibraries());
    ...
    if (minecraftDirectory != null
      ...
      assetIndex =
          new MinecraftLaunchPlan.AssetIndex(
              ...
              displayPath(
                  workingDirectory,
                  installLocator.assetIndexPath(minecraftDirectory, metadata.assetIndex().id())),
    ...
    return new MinecraftLaunchPlan(
        ...
        config.baselineServerEnabled() && config.side() == MinecraftSide.SERVER
            ? metadata.id()
            : null,
        metadata.id(),
        config.side().id(),
        ...
        config.side() == MinecraftSide.CLIENT && clientJar != null
            ? displayPath(workingDirectory, clientJar)
            : null,
  ...
  private List<MinecraftLaunchPlan.Library> toPlanLibraries(
      Path workingDirectory, List<MinecraftLibrarySelector.SelectedLibrary> libraries) {
