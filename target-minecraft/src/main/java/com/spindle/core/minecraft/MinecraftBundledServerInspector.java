...
public final class MinecraftBundledServerInspector {
  ...
  private List<BundledEntry> readList(JarFile jarFile, String entryName, String kind)
    ...
    for (String rawLine : content.split("\\R")) {
      ...
      if (parts.length < 3) {
        throw new LoaderException(
            "Malformed bundled server metadata in " + entryName + " at line " + lineNumber);
      }
      String sha1 = parts[0];
      String id = parts[1];
      ...
      String jarEntryName =
          "library".equals(kind) ? "META-INF/libraries/" + path : "META-INF/versions/" + path;
      if (jarFile.getEntry(jarEntryName) == null) {
        throw new LoaderException(
            "Bundled server metadata references missing entry " + jarEntryName);
      }
      entries.add(new BundledEntry(kind, id, sha1, path, jarEntryName));
    }
    entries.sort(
        Comparator.comparing(BundledEntry::kind)
            .thenComparing(BundledEntry::path)
            .thenComparing(BundledEntry::id));
  ...
  public record Inspection(
      boolean bundled,
      String mainClass,
      List<BundledEntry> versions,
      List<BundledEntry> libraries) {
    public Inspection {
  ...
  public record BundledEntry(
      String kind, String id, String sha1, String path, String jarEntryName) {}
}
