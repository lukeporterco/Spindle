...
This is a public runtime API boundary document. It records the stabilized non-Minecraft Runtime API surface and the runtime honesty metadata exposed to mod developers.

## Inputs

- Runtime-5 loader API boundary inventory.
- Existing public runtime-facing API classes and deferred Minecraft API classes.

## Output

- Runtime API metadata fields on `LoaderApi`.
- Public unchecked API exception classes.
- Boundary status `runtime-api-stabilized` with `nextArc: "Loader API Hardening"`.

## Capability Added Or Recorded

- Stabilizes the Runtime API-0 non-Minecraft surface.
- Exposes explicit metadata saying standard Java mod execution is not sandboxed.
...
- Does not stabilize `com.spindle.api.minecraft.*`.
- Keeps `resource.declare` and `resource.overlay` unavailable.
- Does not turn API metadata into runtime feature flags.

## Follow-On Direction

- Loader API Hardening verifies the same Runtime API version and keeps the boundary synchronized with security shadowing behavior.
