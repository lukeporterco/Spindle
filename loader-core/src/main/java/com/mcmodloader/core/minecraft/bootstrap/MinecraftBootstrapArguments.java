...
public record MinecraftBootstrapArguments(
    ...
    public static MinecraftBootstrapArguments parse(String[] args) throws LoaderException {
        ...
        for (int index = 0; index < args.length; index++) {
            ...
            switch (argument) {
                case "--working-directory" -> workingDirectory = Path.of(requireValue(argument, args, ++index));
                ...
                case "--integration-plan" -> integrationPlanPath = Path.of(requireValue(argument, args, ++index));
                case "--execution-plan" -> executionPlanPath = Path.of(requireValue(argument, args, ++index));
                case "--expected-runtime-fingerprint" -> expectedRuntimeFingerprint = requireValue(argument, args, ++index);
                case "--expected-boundary-fingerprint" -> expectedBoundaryFingerprint = requireValue(argument, args, ++index);
                case "--expected-integration-fingerprint" -> expectedIntegrationFingerprint = requireValue(argument, args, ++index);
                case "--expected-execution-fingerprint" -> expectedExecutionFingerprint = requireValue(argument, args, ++index);
        ...
        if (workingDirectory == null || runtimePlanPath == null || boundaryPath == null || integrationPlanPath == null || executionPlanPath == null) {
            throw new LoaderException("Minecraft bootstrap requires working directory plus runtime, boundary, integration, and execution plan paths.");
        }
    ...
    private static String requireValue(String argument, String[] args, int index) throws LoaderException {
