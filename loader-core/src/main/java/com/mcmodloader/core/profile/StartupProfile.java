...
public record StartupProfile(int schema, long totalDurationMs, List<DiagnosticEvent> events, List<SlowEvent> slowestEvents) {
    public static StartupProfile from(List<DiagnosticEvent> events) {
        ...
        List<SlowEvent> slowestEvents =
            events
                ...
                .sorted(Comparator.comparingLong(SlowEvent::durationMs).reversed().thenComparing(SlowEvent::name))
                .limit(5)
                .toList();
