...
public final class MinecraftBootstrapClassLoaderGraphBuilder {
    public MinecraftBootstrapClassLoaderGraph build(MinecraftModExecutionPlan executionPlan) {
        ...
        for (MinecraftExecutableMod mod : executionPlan.acceptedExecutableMods().stream().sorted(Comparator.comparing(MinecraftExecutableMod::modId)).toList()) {
            nodes.add(
                new MinecraftBootstrapClassLoaderGraph.ClassLoaderNode(
                    ...
                    List.of(mod.plannedDelegationPolicy(), "loader-core packages denied", "loader-api visible")
                )
            );
        }
        return new MinecraftBootstrapClassLoaderGraph(
            ...
            executionPlan.acceptedExecutableMods().stream().map(MinecraftExecutableMod::modJarPath).sorted().toList(),
            executionPlan.minecraftRuntimeClasspathSummary(),
            false
        );
    }
