  ...
  private String permissionFindingMessage(String permission, RuntimeCapabilityGrant grant) {
    return switch (grant.state()) {
      case "denied" ->
          "requests capability `"
              + permission
              + "`, but Runtime-5 marks it `denied`; "
              + grant.controls().replaceFirst("\\.$", "").toLowerCase()
              + ".";
      case "unavailable" ->
          "requests capability `"
              + permission
              + "`, but Runtime-5 marks it `unavailable`; no Spindle API surface is granted for this capability.";
      case "unknown" ->
          "requests capability `"
              + permission
              + "`, but Runtime-5 does not recognize it and grants no Spindle API surface.";
      case "visibility-only" ->
          "requests capability `"
              + permission
              + "`, which Runtime-5 records for review but does not enforce because runtime Java execution remains in-process unrestricted Java.";
      default ->
