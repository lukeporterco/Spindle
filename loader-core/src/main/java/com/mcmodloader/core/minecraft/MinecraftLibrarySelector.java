...
public final class MinecraftLibrarySelector {
    ...
    public Selection select(MinecraftVersionMetadata metadata, Path librariesRoot, OperatingSystem operatingSystem) {
        ...
        for (MinecraftVersionMetadata.Library library : metadata.libraries()) {
            ...
            if (library.artifact() != null && library.artifact().path() != null && !library.artifact().path().isBlank()) {
                libraries.add(
                    new SelectedLibrary(
                        library.name(),
            ...
            MinecraftVersionMetadata.Download classifierDownload = library.classifiers().get(classifierKey);
            if (classifierDownload != null && classifierDownload.path() != null && !classifierDownload.path().isBlank()) {
                nativeLibraries.add(
                    new SelectedLibrary(
                        library.name(),
    ...
    public static boolean allows(List<MinecraftVersionMetadata.Rule> rules, OperatingSystem operatingSystem) {
    ...
    public static boolean matches(MinecraftVersionMetadata.Rule rule, OperatingSystem operatingSystem) {
        if (rule == null) {
            return false;
        }
        if (rule.osName() != null && !rule.osName().isBlank() && !rule.osName().equalsIgnoreCase(operatingSystem.name())) {
            return false;
        }
        if (rule.osArch() != null && !rule.osArch().isBlank() && !operatingSystem.arch().equalsIgnoreCase(rule.osArch())) {
            return false;
        }
        return rule.osVersion() == null || rule.osVersion().isBlank() || operatingSystem.version().matches(rule.osVersion());
    }

    private String nativeClassifierKey(MinecraftVersionMetadata.Library library, OperatingSystem operatingSystem) {
    ...
    public record SelectedLibrary(String name, Path path, String sha1, long size, boolean nativeLibrary) {
    ...
    public record OperatingSystem(String name, String arch, String version) {
        static OperatingSystem detect() {
            ...
            return new OperatingSystem(name, System.getProperty("os.arch", ""), System.getProperty("os.version", ""));
        }
