# MC ModLoader Direction Reference

## Purpose

MC ModLoader should become a forward-only, server-first, deterministic Java 25 mod platform for baseline Minecraft `26.1.2`. It should not try to become a Fabric, Forge, NeoForge, or Paper compatibility layer. Its core identity should be a small, deterministic loader that compiles a modpack into a verified runtime contract before any mod class is touched.

The recommended direction is:

> MC ModLoader is a deterministic modpack runtime compiler first, a lifecycle/service/config/resource platform second, and a hook-capable Minecraft mod loader third.
...
This arc should turn MC ModLoader into a deterministic modpack runtime compiler.
...
MC ModLoader compiles a modpack into a deterministic profile and uses that profile to control lifecycle execution, mod context creation, package ownership, storage ownership, and diagnostics.

Included:

- `loader.mod.json` schema v2 draft
- `mcml.compiled-profile.json`
...
This arc should make MC ModLoader useful and ergonomic as its own ecosystem.
...
- `mcmlCompileProfile`
- `mcmlRunServer`
- `mcmlRunServerOffline`
- `mcmlPrintProfile`
- `mcmlExplainModpack`
- `mcmlCheckReproducibility`
- `mcmlGenerateModTemplate`
...
- `mcml.lock.json`
- `mcml.compiled-profile.json`
- `mcml.lifecycle-report.json`
- `mcml.quality-report.json`
- `mcml.execution-report.json`
- `mcml.reproducibility-report.json`
- `mcml.preflight-report.json`

The `mcml.` prefix makes generated loader artifacts easy to identify in build output and reports.
...
- `com.mcmodloader.api`
- `com.mcmodloader.api.lifecycle`
- `com.mcmodloader.api.config`
- `com.mcmodloader.api.service`
- `com.mcmodloader.api.resource`
- `com.mcmodloader.api.hook`
- `com.mcmodloader.api.diagnostic`
...
MC ModLoader compiles mod metadata into a reusable, deterministic runtime profile before classloading or execution.
