  ...
  public FrozenModGraph build(
    ...
    for (FrozenMod mod : frozenMods) {
      for (Map.Entry<String, String> dependency : mod.depends().entrySet()) {
        ...
        dependencyEdges.add(
            new DependencyEdge(mod.id(), targetId, dependency.getValue(), satisfiedBy));
      }
      ...
      for (Map.Entry<String, String> incompatibility : mod.breaks().entrySet()) {
        ...
        if (targetMod != null) {
          incompatibilityEdges.add(
              new IncompatibilityEdge(
                  mod.id(), targetMod.id(), incompatibility.getValue(), targetMod.version()));
        }
      }
    }

    dependencyEdges.sort(
        Comparator.comparing(DependencyEdge::fromId).thenComparing(DependencyEdge::toId));
    incompatibilityEdges.sort(
        Comparator.comparing(IncompatibilityEdge::fromId).thenComparing(IncompatibilityEdge::toId));
