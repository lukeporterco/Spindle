...
public final class MinecraftArtifactVerifier {
  public VerificationResult verify(Path path, String expectedSha1, Long expectedSize)
    ...
    if (expectedSha1 != null
        ...
        && !expectedSha1.equalsIgnoreCase(hashes.sha1())) {
      throw new LoaderException(
          "Artifact SHA-1 mismatch for "
    ...
    if (expectedSize != null
        && expectedSize.longValue() >= 0L
        && expectedSize.longValue() != hashes.size()) {
      throw new LoaderException(
  ...
  private Hashes computeHashes(Path path) throws LoaderException {
    try {
      ...
      throw new LoaderException(
          "Failed to verify artifact " + path.toString().replace('\\', '/'), exception);
    }
  }
