...
This is a planning-only patch dry run pass document for the Minecraft Target Layer. It records what Target-7 adds or decides while preserving the current Minecraft, SteelHook, and public API boundaries.

## Inputs

- Target-5 placement plan.
- Target-6 instruction-aware bytecode model.
- Internal dispatcher convention for `SteelHookDispatcher.beforeMinecraftServerMain()`.

## Output

- Deterministic `minecraft-hook-patch-plan.json`.

## Capability Added Or Recorded

- Plans one method-entry `invokestatic` dispatcher insertion at bytecode offset `0`.
- Records constant-pool, bytecode, branch, switch, exception-table, and nested `Code` attribute rewrite impact without rewriting bytes.
...
Target-7 remains planning-only. Target-8 is the first pass that may produce transformed class bytes, but only for fixture/test proof. Target-9 may pass the validated Target-7 patch plan into fake-server bootstrap execution, but Target-7 still does not itself transform real Minecraft runtime artifacts, rewrite `StackMapTable`, expose public APIs, add gameplay hooks, use Mixin or Java agents, or imply Java mod execution is sandboxed.

## Boundaries Preserved

- Does not generate transformed class bytes, rewrite the constant pool, rewrite bytecode, install hooks, invoke hooks, expose public APIs, use Mixin or Java agents, or imply sandboxing.

## Follow-On Direction

- Target-8 may produce fixture-only transformed class bytes from the validated dry-run plan.
