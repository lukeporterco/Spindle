...
This is an analysis-only catalog validation pass document for the Minecraft Target Layer. It records what Target-3 adds or decides while preserving the current Minecraft, SteelHook, and public API boundaries.

## Inputs

- Target-1 artifact interpretation.
- Target-2 hook point contract model.
- Minecraft `26.1.2` server-side known-symbol catalog.

## Output

- Schema `2` `minecraft-hook-contracts.json` report with selected catalog metadata and diagnostics.

## Capability Added Or Recorded

- Selects the exact `minecraft-26.1.2-server-known-symbols` catalog for the server side.
- Validates the known `net/minecraft/server/Main.main([Ljava/lang/String;)V` contract against Target-1 symbols.
...
- Does not parse method bytecode, inspect instructions or callsites, classify hook candidates, install hooks, patch classes, load Minecraft classes, expose a public Modding API, or imply Java mod sandboxing.

## Follow-On Direction

- Target-4 uses the validated known-symbol report as its installation gate input.
