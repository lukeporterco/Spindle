...
This is an analysis-only symbol discovery pass document for the Minecraft Target Layer. It records what Target-17 adds or decides while preserving the current Minecraft, SteelHook, and public API boundaries.

## Inputs

- Target-16 resource/reload boundary analysis.
- Target-1 interpreted class, field, and method names/descriptors.

## Output

- Deterministic `minecraft-resource-reload-symbol-analysis.json`.
- `minecraft-resource-reload-binding-analysis.json`.

## Capability Added Or Recorded

- Scans interpreted class/package names plus field and method names/descriptors for fixed resource/reload discovery tokens.
- Reports candidate metadata symbols only.
...
- Does not select a stable reload target, bind reload timing or apply behavior, access resources or datapacks, generate data, mutate registries, expose APIs, add SteelHook primitives, add transformation support, or imply sandboxing.

## Follow-On Direction

- Target-18 can classify binding and access requirements for discovered candidates.
