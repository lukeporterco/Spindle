...
Target-20 now adds the caboose decision pass for that same concept family. It reads Target-16 through Target-19 resource/reload reports, writes `minecraft-resource-reload-arc-decision.json`, records that the current arc is complete enough for now, and chooses registry bootstrap/content registration as the next concept family. It remains analysis-only, does not implement registry behavior, does not discover registry symbols, does not design a new SteelHook primitive, does not implement reload handling, does not access resources or datapacks, does not generate data, does not expose public APIs, and does not imply sandboxing. Target-21 may begin Registry Bootstrap and Content Registration boundary analysis.
...
Target-17 then adds analysis-only symbol discovery on top of that coarse boundary report. It discovers candidate metadata symbols only and feeds Target-18 binding/access strategy analysis, while still not making reload implementation ready by itself.
