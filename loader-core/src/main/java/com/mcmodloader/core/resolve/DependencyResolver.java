...
public final class DependencyResolver {
    ...
    private void validateDependencies(LaunchContext context, ModCandidate candidate, Map<String, ModCandidate> candidateById)
        ...
        for (Map.Entry<String, String> dependency : metadata.depends().entrySet()) {
            ...
            switch (dependencyId) {
                case "loader" -> validateVersionRequirement(metadata.id(), dependencyId, context.loaderVersion(), requirement, true);
                ...
                default -> {
                    ...
                    validateVersionRequirement(metadata.id(), dependencyId, dependencyMetadata.version(), requirement, false);
                }
            }
        }
    }

    private void validateIncompatibilities(ModCandidate candidate, Map<String, ModCandidate> candidateById) throws LoaderException {
        ModMetadata metadata = requireMetadata(candidate);
        for (Map.Entry<String, String> incompatibility : metadata.breaks().entrySet()) {
            ...
            VersionRequirement requirement = parseRequirement(metadata.id(), brokenModId, incompatibility.getValue());
    ...
    private void validateVersionRequirement(
        ...
        VersionRequirement requirement = parseRequirement(requestingModId, dependencyId, requirementText);
    ...
    private VersionRequirement parseRequirement(String requestingModId, String dependencyId, String requirementText) throws LoaderException {
        try {
