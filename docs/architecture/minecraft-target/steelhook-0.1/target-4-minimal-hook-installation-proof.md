...
This is an implementation proof pass document for the Minecraft Target Layer. It records what Target-4 adds or decides while preserving the current Minecraft, SteelHook, and public API boundaries.

## Inputs

- Target-3 known-symbol validation report.
- Frozen execution plan for `net.minecraft.server.Main`.
- Bootstrap child-JVM launch path.

## Output

- Deterministic `minecraft-hook-installation-plan.json`.
- Deterministic `minecraft-hook-installation-result.json` from the bootstrap child JVM.
- `minecraft-hook-placement-plan.json`.

## Capability Added Or Recorded

- Plans and executes exactly one launch-boundary main-wrapper hook for `net.minecraft.server.Main.main(String[])`.
- Invokes Minecraft main through an internal runtime bridge after the Target-3 gate passes.
...
- This is not bytecode injection.
- Does not support any other hook ids, owners, members, descriptors, modes, gameplay hooks, public APIs, or sandbox claims.

## Follow-On Direction

- Target-5 adds method-code placement analysis beside this launch-boundary proof.
