...
public final class MinecraftServerRuntimePlanner {
  ...
  private final MinecraftBundledServerInspector bundledInspector =
      new MinecraftBundledServerInspector();
  private final MinecraftBundledServerResolver bundledResolver =
      new MinecraftBundledServerResolver();
  ...
  public PlannedRuntime plan(
    ...
    MinecraftRuntimeCacheLayout runtimeLayout =
        new MinecraftRuntimeCacheLayout(cache.cacheDirectory(), artifactResolution.metadata().id());
    ...
    if (mode == MinecraftServerRuntimeMode.BUNDLED_SERVER) {
      ...
      command =
          MinecraftServerLaunchCommand.classpath(
              javaExecutable,
              inspection.mainClass() == null || inspection.mainClass().isBlank()
                  ? "net.minecraft.server.Main"
                  : inspection.mainClass(),
              rawClasspath,
              serverJar,
      ...
      command =
          MinecraftServerLaunchCommand.simpleJar(
              javaExecutable, serverJar, config.serverJvmArgs(), config.serverArgs(), displayPath);
    }

    boolean cacheOnly =
        artifactResolution.networkRequestCount() == 0
            && ("cache".equals(artifactResolution.serverJarSource())
                || "local".equals(artifactResolution.serverJarSource()));
    boolean replayableOffline =
        serverJar != null
            && serverRecord != null
            && serverRecord.present()
            && artifactResolution.networkRequestCount() == 0;
    ...
    MinecraftRuntimeProvenance provenance =
        new MinecraftRuntimeProvenance(
            ...
            artifactResolution.networkRequestCount() == 0
                ? "none"
                : "mojang-metadata-or-server-artifact",
    ...
    MinecraftServerRuntimePlan plan =
        new MinecraftServerRuntimePlan(
            ...
            artifactResolution.versionSelection() == null
                ? config.requestedVersionOrBaseline()
                : artifactResolution.versionSelection().requested(),
            artifactResolution.versionSelection() == null
                ? "exact version or explicit version JSON"
                : artifactResolution.versionSelection().source(),
            artifactResolution.manifestRecord() == null
                ? "none"
                : artifactResolution.manifestRecord().status().id(),
            ...
            mode.id(),
            inspection.bundled()
                ? "server jar contains bundled runtime metadata"
                : "server jar has no bundled runtime metadata",
  ...
  public record PlannedRuntime(
      MinecraftServerRuntimePlan plan, MinecraftServerLaunchCommand command) {}

  private String normalizedWorkingDirectory(Path workingDirectory, Path configuredServerDirectory) {
    Path effective =
        configuredServerDirectory == null ? workingDirectory : configuredServerDirectory;
    return workingDirectory
            .toAbsolutePath()
            .normalize()
            .equals(effective.toAbsolutePath().normalize())
        ? "."
        : effective.getFileName().toString();
  }

  private String normalizeExecutable(Path javaExecutable) {
    return javaExecutable == null || javaExecutable.getFileName() == null
        ? "java"
        : javaExecutable.getFileName().toString();
  }
