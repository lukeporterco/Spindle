...
public final class MinecraftModIntegrationPlanner {
  ...
  public MinecraftModIntegrationPlan plan(
      ...
      throws com.spindle.core.diagnostics.LoaderException {
    List<String> discovered =
        parsedCandidates.stream().map(ModCandidate::normalizedRelativePath).sorted().toList();
    ...
    for (ModCandidate candidate :
        parsedCandidates.stream()
            .sorted(java.util.Comparator.comparing(ModCandidate::normalizedRelativePath))
            .toList()) {
      ModMetadata metadata = candidate.metadata();
      String modId = metadata.id();
      MinecraftJarScanResult scan =
          jarScanner.scan(candidate.jarPath(), candidate.normalizedRelativePath());
      ...
      dependencyGraph.put(
          modId,
          metadata.depends().keySet().stream().filter(key -> !isBuiltin(key)).sorted().toList());

      boolean sideOk = "server".equals(metadata.side()) || "universal".equals(metadata.side());
      sideCompatibility.put(
          modId, sideOk ? "accepted for server" : "client-only mod is not server-compatible");
      boolean loaderOk =
          requirementMatches(metadata.depends().get("loader"), LoaderRuntimeInfo.LOADER_VERSION);
      boolean javaOk =
          requirementMatches(
              metadata.depends().get("java"), Integer.toString(context.javaMajorVersion()));
      boolean minecraftOk =
          requirementMatches(metadata.depends().get("minecraft"), resolvedMinecraftVersion);
      loaderCompatibility.put(
          modId, loaderOk ? "ok" : "requires " + metadata.depends().get("loader"));
      javaCompatibility.put(modId, javaOk ? "ok" : "requires " + metadata.depends().get("java"));
      minecraftCompatibility.put(
          modId, minecraftOk ? "ok" : "requires " + metadata.depends().get("minecraft"));
      ...
      minecraftConflicts.addAll(
          boundaryViolations.stream()
              .filter(violation -> violation.type().contains("runtime"))
              .toList());
      loaderConflicts.addAll(
          boundaryViolations.stream()
              .filter(violation -> violation.type().contains("loader"))
              .toList());
      ...
      if (!loaderOk) {
        issues.add(
            new MinecraftBoundaryViolation(
                ...
                modId,
                LoaderRuntimeInfo.LOADER_VERSION,
                true,
                true,
      ...
      boolean fatalBoundaryViolation =
          boundaryViolations.stream()
              .anyMatch(violation -> violation.severity() == MinecraftBoundarySeverity.FATAL);
      if (!sideOk
          || !loaderOk
          || !javaOk
          || !minecraftOk
          || (strictClassVersions && !classVersionOk)
          || fatalBoundaryViolation) {
        String reason =
            !sideOk
                ? "side mismatch"
                : !loaderOk
        ...
        MinecraftBoundarySeverity rejectionSeverity =
            (strictSide && !sideOk) || fatalBoundaryViolation
                ? MinecraftBoundarySeverity.FATAL
                : MinecraftBoundarySeverity.ERROR;
        rejected.add(new MinecraftModRejection(modId, reason, rejectionSeverity, true));
      } else {
        accepted.add(
            new MinecraftModAcceptance(
                modId, metadata.version(), metadata.side(), "metadata and analysis checks passed"));
      }
    }
    ...
    for (String duplicate : duplicateResources) {
      issues.add(
          new MinecraftBoundaryViolation(
              ...
              false,
              true,
              "Resource is owned by multiple mods."));
    }
    for (String splitPackage : splitPackages) {
      issues.add(
          new MinecraftBoundaryViolation(
              ...
              false,
              true,
              "Package is owned by multiple mods."));
    }

    List<String> topologicalOrder =
        resolvedMods.mods().stream()
            .map(ResolvedModSet.ResolvedMod::id)
            .sorted()
            .filter(id -> accepted.stream().anyMatch(mod -> mod.modId().equals(id)))
            .toList();
    List<String> plannedModClasspath =
        accepted.stream()
            .map(MinecraftModAcceptance::modId)
            .sorted()
            .map(id -> "future-mod-classloader:" + id)
            .toList();
    ...
    return new MinecraftModIntegrationPlan(
        ...
        new MinecraftClasspathBoundaryPlan(
            "future child-first mod classloader is planned but not created",
            "loader and Minecraft remain parents; no attachment occurs"),
        packagesByMod,
        resourcesByMod,
        servicesByMod,
        moduleInfoByMod,
  ...
  private boolean isBuiltin(String dependencyId) {
    return "loader".equals(dependencyId)
        || "java".equals(dependencyId)
        || "minecraft".equals(dependencyId);
  }
  ...
  private List<String> conflicts(Map<String, List<String>> ownershipByOwner) {
    ...
    return ownerBySubject.entrySet().stream()
