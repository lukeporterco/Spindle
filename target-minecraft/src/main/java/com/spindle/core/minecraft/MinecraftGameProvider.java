...
public final class MinecraftGameProvider implements GameProvider {
  ...
  public void validate(LaunchContext context) throws LoaderException {
    ...
    if ((resolvedVersion == null || resolvedVersion.isBlank())
        && config.explicitVersionJson() != null
        && Files.isRegularFile(config.explicitVersionJson())) {
      String json;
      try {
        json = java.nio.file.Files.readString(config.explicitVersionJson());
      } catch (java.io.IOException exception) {
        throw new LoaderException(
            "Failed to read Minecraft version JSON: " + config.explicitVersionJson(), exception);
      }
      resolvedVersion =
          metadataParser.parse(json, config.explicitVersionJson().toString(), config.side()).id();
    }
  }
  ...
  public void launch(LaunchContext context, ClassLoader classLoader) throws LoaderException {
    throw new LoaderException(
        "Minecraft launch is intentionally controlled by the managed server runtime path. Use --minecraft-dry-run with explicit Minecraft server planning or launch flags.");
  }
