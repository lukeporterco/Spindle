...
## Command Output and Token Control

Prefer RTK wrappers for noisy commands so command output is filtered before entering model context.
...
- `rtk read <file>`
- `rtk ./gradlew.bat :target-minecraft:test`
- `rtk ./gradlew.bat spotlessCheck`

Do not use RTK to hide failures. If filtered output is insufficient, rerun the narrow failing command or inspect the specific affected file. Avoid repo-wide searches unless the task explicitly requires them.
