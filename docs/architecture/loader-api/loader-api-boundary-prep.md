# Loader API Boundary Prep: Prepared Public API Inventory

This is a boundary-prep document. It records the Runtime-5 public loader-api inventory before the boundary is stabilized by Loader API-0.

## Inputs

- Runtime-5 runtime closure output.
- Current `loader-api` public classes and internal exclusions.

## Output

- `spindle.profile.json.runtimeClosure.loaderApiBoundary` with `status: "prepared-not-sealed"` and `nextArc: "Loader API Arc"`.

## Capability Added Or Recorded

- Records a code-backed inventory of candidate public loader API surfaces.
- Keeps Minecraft-specific API classes deferred for later target-specific review.
...
- Does not seal the final public contract, rename APIs, redesign APIs, or stabilize `com.spindle.api.minecraft.*`.
- Does not use reflection to discover the public classes.

## Follow-On Direction

- Loader API-0 stabilizes the runtime-facing subset and Loader API Hardening verifies compatibility and drift behavior.
